...
assert.ok(js.includes('persistAssistantFeedback'), 'feedback persist helper');
assert.ok(js.includes('setAssistantMessageId'), 'message id binder');
assert.ok(js.includes('/messages/') && js.includes('method: \'PATCH\''), 'PATCH feedback API');
assert.ok(js.includes('msgOpts') || js.includes('feedback: opts.feedback') || js.includes('feedback: msg.feedback'), 'restore feedback');
